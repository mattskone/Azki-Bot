...
def write_reply(request_parts):
    ...
    if video == None:
        return f"I can't seem to find {request_parts[0]} singing that song. Sorry about that :("
...
def main():
    ...
    TODO: Reddit-related exception handling
          Prevent multi comment when restarting app
    """
    reddit = praw.Reddit(
    ...
    subreddit = reddit.subreddit("okbuddyhololive")
    ...
    for raw_comment in subreddit.stream.comments():
        ...
        if trigger in raw_comment.body:
            comment = raw_comment.body.lower().replace(",", "")
            comment_as_list = comment.split(" ")
            trigger_index = comment_as_list.index("play")
            if comment_as_list[trigger_index - 1] in vtuber_list:  # If it's not a whitelisted VTuber, ignore it
                key_elements = comment_parse(comment_as_list, trigger_index)
